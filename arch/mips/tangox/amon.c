...
{
	struct cpulaunch *launch = (struct cpulaunch *)CKSEG1ADDR(CPULAUNCH);

	if (cpu < 0 || cpu >= NCPULAUNCH) {
		pr_debug("avail: cpu%d is out of range\n", cpu);
		return 0;
	} else if (cpu == 0)
		return 1;	/* CPU0 is always available */
		    ...
		    unsigned long gp, unsigned long a0)
{
	volatile struct cpulaunch *launch =
		(struct cpulaunch  *)CKSEG1ADDR(CPULAUNCH);
...
void amon_cpu_dead(void)
{
	...
	launch = (struct cpulaunch *)CKSEG1ADDR(CPULAUNCH);
	...
	cpufreq = 800000000;	/* FIXME: Correct value? */
