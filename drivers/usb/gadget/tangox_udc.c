...
MODULE_PARM_DESC (controller, "TANGOX Controller 0 or 1");

static const char driver_name [] = "tangox_otg";
...
#if 0
static void free_prev_dtd(struct tangox_otg *dev, struct ep_qh * dqh)
{
	struct dtd *dtd = 0;
...
static int tangox_udc_init(struct tangox_otg *dev, u8 inited)
{
	...
	/*set it to dev mode*/
	temp = gbus_read_uint32(pGBus, tangox_ehci_base[ctrl] + TANGOX_EHCI_REG_OFFSET +0xA8);
	temp |= 2 ;
...
static int tangox_ep_irq (struct tangox_otg *dev)
{
	...
	if (setup_stat) {
		...
		DEBUG("02-------%s ep_setupstat %04x ep_num=0x%x---\n",
		...
		DEBUG("01-------%s ep_setupstat %04x ep_num=0x%x---\n",
		 __FUNCTION__, setup_stat, ep_num);
	...
	if (!setup_stat) {
        	DEBUG("1--------setup packet--%s ep_setupstat %04x done--\n\n",
	...
	switch (cr->bRequest) {
		case USB_REQ_SET_ADDRESS: {
			DEBUG("USB_REQ_SET_ADDRESS 0x%x\n", cr->bRequest);
...
out:
    ...
    DEBUG("2-------setup packet--%s ep_setupstat %04x done---\n\n",
		 __FUNCTION__, setup_stat);
	return 0;
}
...
static int tangox_remove (struct platform_device *pdev)
{
	...
	if (((chip_id == 0x8672) || (chip_id == 0x8674)) && (controller == 1))
		free_irq(IRQ_CONTROLLER_IRQ_BASE + LOG2_CPU_DVD_INT, dev);
	else 
		free_irq(tangox_ehci_irq[controller], dev);
...
static int tangox_probe (struct platform_device *pdev)
{
    ...
    base =  (void *)NON_CACHED(tangox_ehci_base[controller]);
    dev->cap_regs   = (struct tangox_cap_regs   __iomem *) (base + TANGOX_EHCI_REG_OFFSET);
    dev->regs       = (struct tangox_op_regs    __iomem *) (base +  TANGOX_EHCI_REG_OFFSET+0x40);

	irq = tangox_ehci_irq[controller];
	if (((chip_id == 0x8672) || (chip_id == 0x8674)) && (controller == 1))
		irq = IRQ_CONTROLLER_IRQ_BASE + LOG2_CPU_DVD_INT;

	if (request_irq(irq, (void *)tangox_irq, IRQF_DISABLED, driver_name, dev)
			!= 0) {
		printk("request interrupt %d failed\n", tangox_ehci_irq[controller]);
		retval = -EBUSY;
		goto done;
	}
	...
	printk ("IRQ %d, MEM 0x%x, Chip Rev. %04x\n",
		 tangox_ehci_irq[controller], (u32)base, dev->chiprev);
	the_controller = dev;
...
static u64 tangox_dmamask = ~(u32)0;
static void tangox_udc_release_dev(struct device * dev)
{
        dev->parent = NULL;
}

static struct platform_device tangox_udc_device = {
	.name           = (char *)driver_name,
	.id             = -1,
	.dev = {
		.dma_mask               = &tangox_dmamask,
		.coherent_dma_mask      = 0xffffffff,
		.release                = tangox_udc_release_dev,
	 },
	.num_resources  = 0,
	.resource       = 0,
};
...
static int __init init (void)
{
	...
	return platform_driver_register(&tangox_udc_driver);
}
...
static void __exit cleanup (void)
{
	...
	platform_driver_unregister(&tangox_udc_driver);
	platform_device_unregister(&tangox_udc_device);
