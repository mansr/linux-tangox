...
#ifdef CONFIG_TANGO4
static int ir_irq = LOG2_SYS_GPIO_INFRARED_INT + IRQ_CONTROLLER_IRQ_BASE;
#else
	...
	switch(cmd) {
		case IR_IOCSETREPEATKEYS: /* Set the repetition keys */
			if (copy_from_user(&key_no, ptr, sizeof(unsigned long))) 
				...
				goto einval_out;
			else if (copy_from_user(tab, ptr + 1, sizeof(unsigned long) * key_no)) 
				goto efault_out;
			spin_lock_irqsave(&priv->lock, flags);
			priv->num_keys = key_no;
			memcpy(priv->key_table, tab, sizeof(unsigned long) * key_no);
			spin_unlock_irqrestore(&priv->lock, flags);
			break;
		case IR_IOCGETREPEATKEYS: /* Get the repetition keys */
			spin_lock_irqsave(&priv->lock, flags);
			key_no = priv->num_keys;
			memcpy(tab, priv->key_table, sizeof(unsigned long) * key_no);
			spin_unlock_irqrestore(&priv->lock, flags);
			if (copy_to_user(ptr, &key_no, sizeof(unsigned long))) 
				goto efault_out;
			else if (key_no > 0) {
				if (copy_to_user(ptr + 1, tab, sizeof(unsigned long) * key_no)) 
					goto efault_out;
			}
			break;
		...
		case IR_IOCGETWAITPERIOD: 
			...
			tmp = wait_period;
			spin_unlock_irqrestore(&priv->lock, flags);
			if (copy_to_user(ptr, &tmp, sizeof(int))) 
				goto efault_out;
			break;
		default:
			goto einval_out;
