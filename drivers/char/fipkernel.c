...
static DECLARE_WAIT_QUEUE_HEAD(fip_wq);
...
static long fip_unlocked_ioctl(struct file *, unsigned int, unsigned long);
static unsigned int fip_poll(struct file *, struct poll_table_struct *);
#ifdef CONFIG_TANGO4
static int fip_irq = LOG2_SYS_GPIO_FIP_INT + IRQ_CONTROLLER_IRQ_BASE;
#else
static int fip_irq = LOG2_CPU_FRONTPANEL_INT + IRQ_CONTROLLER_IRQ_BASE;
#endif
static struct cdev cdev;
static struct class *fip_class;
...
static struct file_operations fip_fops = {
	open: fip_open,
	read: fip_read,
	unlocked_ioctl: fip_unlocked_ioctl,
	poll: fip_poll,
	write: fip_write,
	release: fip_release,
	owner: THIS_MODULE,
...
static irqreturn_t fip_isr(int irq, void *dev_id)
{
	...
	if (irq == fip_irq) {
		//gbus_write_reg32(REG_BASE_CPU + CPU_edge_rawstat,  IRQMASKOF(irq));
...
static long fip_unlocked_ioctl(struct file *fptr, unsigned int cmd, unsigned long arg)
{
	int on = 0;
	int symbol;
	int i, j, k;
...
static unsigned int fip_read_reg(unsigned int offset)
{
	unsigned int val = gbus_read_reg32(fip_base + offset);
	return(val);
}
...
{
	fip_wait_ready();
	gbus_write_reg32(fip_base + offset, val);
	fip_wait_ready();
}
...
int __init fip_init_module(void)
{
	...
	if (major_num) {
		...
		status = register_chrdev_region(devid, 1, "fip");
	} else {
		status = alloc_chrdev_region(&devid, 0, 1, "fip");
	...
	if (IS_ERR(fip_class = class_create(THIS_MODULE, "fip_device"))) {
		printk(KERN_ERR "%s: error creating fip_device class.\n", fip_devname);
		cdev_del(&cdev);
		unregister_chrdev_region(MKDEV(major_num, 0), 1);
		kfree(fip_priv.buffer);
		return(status);
	}
	if (device_create(fip_class, NULL, MKDEV(major_num, 0), NULL, "fip") == NULL) {
		printk(KERN_ERR "%s: error creating fip_device.\n", fip_devname);
		class_destroy(fip_class);
		cdev_del(&cdev);
		unregister_chrdev_region(MKDEV(major_num, 0), 1);
		kfree(fip_priv.buffer);
		return(-EIO);
	...
	/* Hook up ISR */
	if (request_irq(fip_irq, fip_isr, IRQF_DISABLED, fip_devname, &fip_priv) != 0) {
		printk(KERN_ERR "%s: cannot register IRQ (%d)\n", fip_devname,
			fip_irq);
		device_destroy(fip_class, MKDEV(major_num, 0));
		class_destroy(fip_class);
		cdev_del(&cdev);
		unregister_chrdev_region(MKDEV(major_num, 0), 1);
		kfree(fip_priv.buffer);
	...
	printk("SMP8xxx %s (%d:0): driver loaded (buffer_size = %d)\n", 
