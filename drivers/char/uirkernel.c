...
#define MEM1_FULL		0x01000000
#define MEM2_FULL		0x02000000
...
static long uir_unlocked_ioctl(struct file *, unsigned int, unsigned long);
...
static struct file_operations uir_fops = {
	open: uir_open,
	read: uir_read,
	unlocked_ioctl: uir_unlocked_ioctl,
	poll: uir_poll,
	release: uir_release,
	owner: THIS_MODULE,
};
...
#ifdef CONFIG_TANGO4
static int uir_irq = LOG2_SYS_GPIO_INFRARED_INT + IRQ_CONTROLLER_IRQ_BASE;
#else
...
static int uir_open(struct inode *inode_ptr, struct file *fptr)
{
	...
	priv->ref_cnt++;
	priv->b_mode = ((fptr->f_flags & O_NONBLOCK) ? 0 : 1);
...
static long uir_unlocked_ioctl(struct file *fptr, unsigned int cmd, unsigned long arg)
{
	struct uir_private *priv = (struct uir_private *)fptr->private_data;
...
int __init uir_init_module(void)
{
	...
	if (major_num) {
		...
		status = register_chrdev_region(devid, enabled_uir, "uir");
	} else {
		status = alloc_chrdev_region(&devid, 0, enabled_uir, "uir");
	...
	cdev_init(&cdev, &uir_fops);
	cdev.owner = THIS_MODULE;
	cdev.ops = &uir_fops;
	if ((status = cdev_add(&cdev, devid, enabled_uir)) < 0) {
		printk(KERN_ERR "%s: cannot get major number %d %d\n", uir_devname, status, major_num); 
		goto err2_out;
	}
	if (IS_ERR(uir_class = class_create(THIS_MODULE, "uir_devices"))) {
		printk(KERN_ERR "%s: error creating uir_devices class.\n", uir_devname);
		goto err3_out;
	}
	for (i = 0; i < enabled_uir; i++) {
		if ((uir_privs[i]->device = device_create(uir_class, NULL, MKDEV(major_num, i), NULL, "uir%d", i)) == NULL) {
			printk(KERN_ERR "%s: error creating uir_device(s).\n", uir_devname);
			goto err4_out;
		}
	}
	...
	printk("%s: SMP8xxx uir (%d:0) driver loaded (bufsize=%d, queuesize=%d, enabled_uir=%d)\n", 
...
err5_out:
	for (i = 0; i < enabled_uir; i++) {
		...
		if (uir_privs[i]->device != NULL)
			device_destroy(uir_class, MKDEV(major_num, i));
	}

err4_out:
	class_destroy(uir_class);

err3_out:
	cdev_del(&cdev);

err2_out:
	unregister_chrdev_region(MKDEV(major_num, 0), enabled_uir);
